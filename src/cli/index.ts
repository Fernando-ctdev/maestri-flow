...
export function createCli(deps: CliDeps = {}) {
  ...
  cli.command("configure").option("-l, --language <language>", "UI language (pt-BR or en)").action(async (_options, command) => { const { runConfigureTui } = await import("../tui/app.js"); await runConfigureTui({ language: resolveLanguage({ flag: command.opts().language ?? cli.opts().language }) }); });
