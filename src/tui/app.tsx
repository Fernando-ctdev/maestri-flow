...
type TuiActions = Partial<Record<Exclude<ScreenId, "connect" | "review">, TuiAction>> & { connect?: (config: WireConfig) => void | Promise<void>; confirmProposal?: (proposal: ProvisioningProposal) => void | Promise<void>; discardProposal?: () => void | Promise<void> };
...
export function App({ initial, onQuit, language = resolveLanguage(), actions, wireConfig, configPath }: { initial?: ConfigureState; onQuit?: () => void; language?: Language; actions?: TuiActions; wireConfig?: WireConfig; configPath?: string }) {
  ...
  useInput((input, key) => {
    if (key.escape) { if (form) { setForm(undefined); setFeedback(text.cancelled); } else if (state.screen === "provisioning" && briefForm !== undefined) { setBriefForm(undefined); setBriefBuffer(""); setFeedback(text.cancelled); } else if (state.screen === "review" && proposal) { const discard = actions?.discardProposal; if (discard) void Promise.resolve(discard()).catch(error => setFeedback(`${text.error}: ${error instanceof Error ? error.message : String(error)}`)); setState(s => ({ ...s, screen: "provisioning", stagedProvisioningProposal: undefined })); setFeedback(text.cancelled); } else onQuit?.(); return; }
    ...
    if (key.return) { const screen = state.screen as ScreenId; if (screen === "connect") { if (connectionReady) { go(screens.findIndex(x => x[0] === "provisioning")); } else openForm(); return; } if (screen === "provisioning" && briefForm === undefined) { if (!state.workspace || !state.discovery || !listProvisioners(state.discovery).length) { setFeedback(text.blocked.provisioning ?? text.disabled); return; } setBriefForm(""); setBriefBuffer(""); setFeedback(undefined); return; } if (screen === "review" && proposal) { const action = actions?.confirmProposal; if (!action) { setFeedback("Persistência da fila indisponível; proposta não aplicada"); return; } setState(s => ({ ...s, connection: "loading", error: undefined })); void Promise.resolve(action(proposal)).then(() => { setState(s => ({ ...s, connection: "connected", stagedProvisioningProposal: undefined, screen: "provisioning", error: undefined })); setFeedback("Proposta aplicada na fila"); }).catch(error => { setState(s => ({ ...s, connection: "error", error: error instanceof Error ? error.message : String(error) })); setFeedback(`${text.error}: ${error instanceof Error ? error.message : String(error)}`); }); return; } const action = actions?.[screen as Exclude<ScreenId, "connect" | "review">]; if (!action) { setFeedback(text.blocked[screen] ?? text.disabled); return; } try { const result = action(); if (result instanceof Promise) void result.then(value => { if (screen === "canvas" && value && typeof value === "object" && "discovery" in value && "workspace" in value) { const canvas = value as CanvasActionResult; setState(s => ({ ...s, discovery: canvas.discovery, workspace: canvas.workspace })); } setFeedback(text.feedback[screen]); }).catch(error => setFeedback(`${text.error}: ${error instanceof Error ? error.message : String(error)}`)); else setFeedback(text.feedback[screen]); } catch (error) { setFeedback(`${text.error}: ${error instanceof Error ? error.message : String(error)}`); } return; }
  ...
  const body = state.screen === "review" && proposal ? `Staged proposal: ${proposal.workItems.map(item => `${item.key}: ${item.title}`).join(" | ")}. Enter confirms; Esc rejects.` : text.body[state.screen] ?? text.body.connect;
  const action = state.screen === "connect" || state.screen === "review" ? undefined : actions?.[state.screen as Exclude<ScreenId, "connect" | "review">];
